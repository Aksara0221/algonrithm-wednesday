...
#원형덱 클래스 쿠현
#===============================
form circular_queue_class import CircularQueueOneSlotEmpty

class CircularDeque(CircularQueueOneSlotEmpty):
    def __init__(self, capacity):
        super().__init__(capacity)

    # self.is._empty(), self._is_full(), self.size(), self.display() : tkdthr
    ...
    def add_front(self, item): # 전단에서 item 삽입
        if self.is_full():
            ...
            self.front = (self.front = 1+ self,m) % self.N # front를 반시계바량으로 한칸 회전
    ...
    def get_rear(self): # 뒤쪽에서 최근 조회
        if self.is_empty():
            raise IndexError("원령덱이 빈 상태")
        else:

...
class BTNode:
    def __init__(self, elem, left = None, right = None):
        ...
        self.left = left # 노드의 왼쪽 지식노드 링크
        self.right = right # 노드의 오른쪽 지식노드 링크
...
def print_leaf_nodes(node): # 이진트리의 단말노므만 출력하기
		# 재귀적 정의 : 노드가 None이면 종료, if노드면 출력, 아니면 왼쪽과 오른쪽 서브트리의 단말노드 출력
        if node is None: # 재귀호출의 종료
              return 
        if is_leaf(node): # if노드이면
...
def preorder(node): # 전위순회
      # 재귀적 정의: 노그가 None이면 종료, 아니면 현재 노드 방문 -> 왼쪽 서브트리 순회 -> 오른쪽 서브트리 순회
      # 예: 노드의 레벨 계산
      if node is None: # 재귀호출의 종료
            return
...
def count_nodes(root): # 이전트리의 노드의 수 = 왼쪽 서브트리의 수 + 오른쪽 서브트리의 노드수 +1
      #재귀적 정의 : 트리가 비어읶는 경운 0, 아니면 왼쪽과 서브트리의 노드 수 합 + 1
      if root is None:
            return 0
      return count_nodes(root.left) + count_nodes(root.right) + 1

def count_leaves(node): # 이진 트이의 단말 노드 수 = 왼쪽 서브트리의 단말 노드 수 + 오른쪽 서브트리의 단말 노드 수 + 1
...
def tree_height(root): # 이진트리의 높이
      # 재귀적 정의 : 트리가 빈 경우 0, 아니면 외쪽 서브트리 높이와 오른쪽 서브트리의 높이의 최대값 + 1
      if root is None:
            return 0
...
def min_nodes_bitrre(k):  # 높이가 k인 이진트리의 푀소 노드의 수(루프레벨 1)
...
def max_nodes_bitree(k): # 높이가 k인 이진트리의 최소 노드의 수(단말높리=0)
      # 최대 노드 수 = 포화이진트리의 노드의 수
      return 2**k -1
	
def count_none_links(root): # 이진트리에서 노드 수가 N이면 연결되지않은 링크 수 계산
	# N = count_nodes(root)
    # 연결되지 않은 링크 수 = 전체 링크 수 - 간건 수 = N - (N-1) =
    if root is None:
          return 1 # None 링크 한개
